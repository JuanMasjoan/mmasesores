...
CREATE TABLE IF NOT EXISTS cartera_cliente.contacto_cliente (
  ...
  id_dni VARCHAR(100) NOT NULL,
  PRIMARY KEY (id_contacto_cliente),
  CONSTRAINT fkcontactocliente_id_dni FOREIGN KEY (id_dni) REFERENCES cartera_cliente.cliente (id_dni) ON DELETE CASCADE )
;
...
CREATE TABLE IF NOT EXISTS cartera_cliente.siniestros (
  ...
  descripcion_siniestro VARCHAR(100) NULL,
  siniestro_ultimo_update DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP,
  PRIMARY KEY (id_siniestro),
  CONSTRAINT fksiniestro_id_cia FOREIGN KEY (id_cia) REFERENCES cartera_cliente.cia (id_cia),
  CONSTRAINT fksiniestro_id_poliza FOREIGN KEY (id_poliza) REFERENCES cartera_cliente.polizas (id_poliza) ON DELETE CASCADE,
  CONSTRAINT fksiniestro_id_ramo FOREIGN KEY (id_ramo) REFERENCES cartera_cliente.ramo (id_ramo),
